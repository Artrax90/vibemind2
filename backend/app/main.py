...
"""
    ...
    # 7. Финальный запрос к LLM (Telegram-bot style)
    prompt = f"""Вы — поисковый робот. Твоя задача — вывести результаты поиска по заметкам.
...
2. Выведи список релевантных заметок:
   - Формат: 
     1. [Название заметки]
     [Здесь краткая цитата ИЛИ фраза "[Содержимое защищено паролем]", если заметка закрыта]
3. Если заметок нет, напиши: "Я не нашел информации по запросу «{req.message}» в ваших заметках."
4. НИКАКИХ вступлений, пояснений и вежливых фраз. Только список.
