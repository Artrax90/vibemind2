...
ИНСТРУКЦИИ:
1. Отвечай подробно и полезно на основе заметок.
2. Если в заметках НЕТ ответа на вопрос и они явно не релевантны, скажи "Я не нашел точной информации по этому вопросу в заметках, но постараюсь помочь:" и ответь из общих знаний. Если заметка имеет статус "[ЗАКРЫТО ПАРОЛЕМ]", обязательно упомяни её, сказав, что ответ может быть там.
3. Обязательно используй [1], [2] для ссылок на заметки.
4. В конце ответа добавь список ID ВСЕХ упомянутых заметок в формате: "SOURCES: ID1, ID2". Это КРИТИЧЕСКИ важно для отображения цитат.
